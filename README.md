...
Полное практическое руководство, готовые пресеты и инструменты для комфортной игры с модами **ArdysaMods** в Dota 2.

Репозиторий архитектурно разделён на **два модуля**:
1. **📘 МОДУЛЬ 1 (Ниже):** Пошаговый гайд по установке и настройке скинов/окружения через официальный лаунчер **ArdysaModsTools** *(для всех игроков — ничего вырезать не нужно!)*.
2. **⚙️ МОДУЛЬ 2 (Продвинутый):** Инструментарий для ручной/скриптовой очистки VPK и экстремального сжатия архива с 15+ ГБ до <1 ГБ ([Перейти к Модулю 2 ➔](./docs/MODULE_2_VPK_OPTIMIZER.md)).
...
>
> 💡 **ВАЖНО:** В актуальных версиях ArdysaModsTools **не нужно ничего вырезать вручную** — выбор, комбинирование и отключение сетов происходят прямо в окне программы!
...
> [!NOTE]
> В лаунчере **больше не нужно вручную модифицировать файлы игры или удалять папки heroes**. Все скины переключаются прямо в графическом интерфейсе.
...
3. Для каждого персонажа вы можете:
   ...
   * **Сбросить на дефолт (Default):** если вы хотите вернуть базовый облик героя без мода, просто выберите стандартный скин. Лаунчер автоматически исключит кастомные модели.
...
# ⚙️ МОДУЛЬ 2: Хирургическая оптимизация VPK (Для продвинутых)

> ⚠️ **Дисклеймер:** Если вам просто нужны скины и ландшафт в игре — используйте **Модуль 1** выше.
>
> Модуль 2 предназначен для **разработчиков, моддеров и владельцев слабых ПК/медленных накопителей**, которым требуется:
> 1. **Физически уменьшить вес архива VPK** с 12–18 ГБ до менее чем 1 ГБ (мгновенная загрузка игры).
> 2. **Удалить файлы неиспользуемых персонажей** из VPK по белому списку (`vpk_mod_config.json`).
> 3. **Сохранить ссылочную целостность `scripts/items/items_game.txt`** и Econ Schema Valve.

### 🛠️ Материалы и документация Модуля 2:
* 📖 [**Полное техническое руководство по хирургической оптимизации VPK**](./docs/MODULE_2_VPK_OPTIMIZER.md)
* 🗂️ [**Справочник внутренней структуры архивов VPK Dota 2**](./docs/vpk_structure.md)
* 📜 **Скрипты автоматизации:**
  * [`scripts/build_filtered_tree.ps1`](./scripts/build_filtered_tree.ps1) — сборка чистого дерева ассетов по манифесту.
  * [`scripts/optimize.ps1`](./scripts/optimize.ps1) — in-place зачистка каталогов.
  * [`scripts/analyze_items_game_refs.py`](./scripts/analyze_items_game_refs.py) — анализатор ссылок `items_game.txt`.
...
* [`presets/skins_preset.json`](./presets/skins_preset.json) — готовый профиль выбранных сетов и Аркан на героев.
...
* [VPKEdit Releases](https://github.com/craftablescience/VPKEdit/releases) — CLI и GUI для работы с VPK архивами Valve.
